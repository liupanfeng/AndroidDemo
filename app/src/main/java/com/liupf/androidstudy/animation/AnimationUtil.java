...
package com.liupf.androidstudy.animation;
...
import com.liupf.androidstudy.App;
...
//    }
    ...
    private static void postAnimation(Runnable runnable) {
        if (mHandler == null) mHandler = new Handler(App.getInstance().getMainLooper());
...
//        mHandler.removeCallbacksAndMessages(null);
    ...
    public static void showDetailAnimotion(final Context context,
        ...
        if (helper == null || mImageView == null) return;
        postAnimation(new Runnable() {
            ...
            public void run() {
                ...
                int targetX = detail ?
                        getWidth(context) / 2 - dip2px(context, imgLength) / 2 - dip2px(context, 18) :
                        getWidth(context) - mImageView.getLayoutParams().width / 2;
                int targetY = detail ?
                        dip2px(context, MARGIN_TOP) : 0;
                int targetSize = detail ?
                        dip2px(context, imgLength) : 0;
                ...
                PropertyValuesHolder iconAlpha = PropertyValuesHolder.ofFloat(View.ALPHA,
                        1f, detail ? 1f : 0.1f);
                ObjectAnimator animationLocation = ObjectAnimator
                        .ofPropertyValuesHolder(mImageView, iconX, iconY, iconAlpha);

                PropertyValuesHolder pvhScaleX = PropertyValuesHolder.ofFloat(View.SCALE_X, 1f, targetScale);
                PropertyValuesHolder pvhScaleY = PropertyValuesHolder.ofFloat(View.SCALE_Y, 1f, targetScale);
                ObjectAnimator animationSize = ObjectAnimator
                        .ofPropertyValuesHolder(mImageView, pvhScaleX, pvhScaleY);

                AnimatorSet animatorSet = new AnimatorSet();
                animatorSet.setInterpolator(new AccelerateDecelerateInterpolator());
                animatorSet.setStartDelay(200);
                animatorSet.setDuration(detail ? 300 : 600);
        ...
        postAnimation(new Runnable() {
            ...
            public void run() {
                ...
                ViewPropertyAnimator animator = view.animate()
                        .translationY(show ? view.getLayoutParams().height : 0)
    ...
    public static void callDialAnimator(final View view, final int visibleState) {
        if (view == null) return;
        postAnimation(new Runnable() {
            @Override
            public void run() {
...
//                    final int targetY = iconLocation[1] + defaultSize - (Util.isMiui() ?
//                            getStatusBarHeight(context) : 0);
        ...
        postAnimation(new Runnable() {
            ...
            public void run() {
                ...
                animatorSet.addListener(new AnimatorListener() {
                    ...
                    public void onAnimationEnd(Animator arg0) {
                        // TODO Auto-generated method stub
                        if (helper != null) helper.onAnimatorEnd(arg0);
                    }
